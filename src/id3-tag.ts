...
function removeTagsFromBuffer(data: Buffer) {
  return removeId3Tag(data)
}

/**
 * Replace tags within the file buffer
 */
export function writeInBuffer(tags: Buffer, buffer: Buffer) {
  buffer = removeTagsFromBuffer(buffer) || buffer
  return Buffer.concat([tags, buffer])
}

/**
 * Creates a tag buffer from the provided tags
 */
